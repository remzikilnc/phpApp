...
class Request
{
    ...
    public function all(): array
    {
        ...
        if (!empty($content) && !json_decode($content, false)) {
            $content = [];
            $this->parse_raw_http_request($content);
        } else if (empty($content)) {
            $content = [];
        } else {
            $content = json_decode($content, true);
        }
        return array_merge($this->request->all() ?? [], $content ?? null, $this->files->all() ?? []);
    }

    protected function parse_raw_http_request(array &$a_data): void
    {
        // read incoming data
